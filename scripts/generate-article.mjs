...
products:
  - name: 製品名
    ...
    affiliate:
      ...
      rakuten: ""
chart:                # 図版（横棒グラフ）に自動変換される。必ず入れること。
  title: "グラフの見出し（何を比べているか）"
  rows:               # 3〜6行。value は棒の長さに使う数値（単位なしの生の数）
    - label: "行の名前（例: 5W）"
      value: 1358
      display: "約1,358円"   # 図に表示する文字列（単位つき）
      highlight: false       # 特に注目させたい行だけ true
  caption: "図の下に置く2〜3文の説明。図から何が読み取れるかを書く。"
  note: "※数値の出どころと前提条件（試算なのか公開仕様なのか）"
---
...
- **すべての型で <!-- FIGURE --> を本文に1回入れる**。frontmatter の chart が
  この位置に図版として展開される。数字を比較している節の直後に置くとよい。
...
- **文字数の目安: 5000〜7000字**（ビルド時に4,500字未満は公開を止める）（見出しだけ多くて中身が薄い構成は不可）
...
- **図版を1点入れる**（frontmatter の chart ＋ 本文の <!-- FIGURE -->）。
  記事の中で最も判断に効く数字の比較をグラフにする。文章の言い換えではなく、
  「表を見ないと分からなかったこと」がひと目で分かるものにすること。
